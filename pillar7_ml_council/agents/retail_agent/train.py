...
from pillar7_ml_council.agents.retail_agent.feature_view import get_retail_feature_columns
from pillar7_ml_council.agents.retail_agent.label_policy import (
    RETAIL_TARGET_COLUMN,
    RETAIL_TRAP_COLUMN,
    build_retail_targets,
)
...
try:
    ...
    raise ImportError(
        "scikit-learn is required for retail_agent/train.py. "
...
def _prepare_retail_dataset(
    dataset: pd.DataFrame,
    target_mode: str = "long_vs_no_action",
) -> pd.DataFrame:
    df = dataset.copy()

    if RETAIL_TARGET_COLUMN not in df.columns or RETAIL_TRAP_COLUMN not in df.columns:
        df = build_retail_targets(df)

    if target_mode == "long_vs_no_action":
        df = df[df[RETAIL_TARGET_COLUMN].isin([0, 1])].copy()
        df["binary_target"] = (df[RETAIL_TARGET_COLUMN] == 1).astype(float)
    elif target_mode == "short_vs_no_action":
        df = df[df[RETAIL_TARGET_COLUMN].isin([0, 2])].copy()
        df["binary_target"] = (df[RETAIL_TARGET_COLUMN] == 2).astype(float)
    else:
        raise ValueError("target_mode must be 'long_vs_no_action' or 'short_vs_no_action'")

    if len(df) == 0:
        raise ValueError("no rows available after retail target filtering")
...
def _build_feature_frames(
    ...
    feature_cols = [c for c in get_retail_feature_columns() if c in train_df.columns]

    if not feature_cols:
        raise ValueError("no retail feature columns found in dataset")
...
def _assert_split_has_both_classes(y: pd.Series | Any, split_name: str) -> None:
    ...
    if info["n_unique"] < 2:
        raise ValueError(
            ...
            f"You need both NO_ACTION and target class in this split."
        )


def train_retail_baseline(
    dataset: pd.DataFrame,
    *,
    target_mode: str = "long_vs_no_action",
    model_name: str = "retail_policy_rf",
    version: str = "v1",
) -> Dict[str, Any]:
    _validate_dataset(dataset)
    _ensure_artifact_dir()

    df = _prepare_retail_dataset(dataset, target_mode=target_mode)
    ...
    registry_record = registry.register_model(
        ...
        train_summary={
            ...
            "trap_rate_train": float(train_df[RETAIL_TRAP_COLUMN].mean()) if RETAIL_TRAP_COLUMN in train_df.columns else None,
            "trap_rate_validation": float(val_df[RETAIL_TRAP_COLUMN].mean()) if RETAIL_TRAP_COLUMN in val_df.columns else None,
            "trap_rate_test": float(test_df[RETAIL_TRAP_COLUMN].mean()) if RETAIL_TRAP_COLUMN in test_df.columns else None,
        },
        split_summary=summarize_splits(splits),
        feature_columns=x_train.columns.tolist(),
        target_column="binary_target",
        notes=f"Retail baseline {target_mode} model",
    )
