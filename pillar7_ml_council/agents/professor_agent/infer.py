...
def _prepare_single_row_dataframe(professor_feature_row: Dict[str, Any]) -> pd.DataFrame:
    row_df = pd.DataFrame([professor_feature_row])
...
def infer_professor_from_feature_row(
    ...
    professor_view = build_professor_feature_row(feature_row)
    row_df = _prepare_single_row_dataframe(professor_view)
    ...
    return {
        ...
        "professor_feature_view": professor_view,
    }


def infer_professor_from_shared_state(
    shared_state: Dict[str, Any],
    artifact_path: str | Path,
    threshold: float = 0.5,
) -> Dict[str, Any]:
    feature_row = build_feature_row(shared_state)
    return infer_professor_from_feature_row(
...
def summarize_professor_inference(result: Dict[str, Any]) -> Dict[str, Any]:
    return {
